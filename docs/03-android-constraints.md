...
- The **foreground service** uses the `specialUse` FGS type plus a partial wakelock while a session is active. We avoid the dataSync and mediaProcessing types because they carry 6h and 24h caps on Android 15+. The wakelock is held only while there is something to keep awake: once the supervisor has given up on restarting a dead server it releases it, because the only way back from there is the user running Repair, and a healthy check re-acquires it.
- **Process monitors do not work in the guest.** Android denies apps `/proc/stat`, so `htop` exits with "Cannot open /proc/stat: Permission denied" and `top` fails the same way. `/proc` is bound into the guest and most of it reads fine — `/proc/meminfo`, `/proc/self/*` — but the per-CPU accounting these tools need is not ours to read. This is the platform, not PRoot, and it is permanent for the same reason Docker is: an unprivileged app does not get it. Verified on device.
- **Nothing polls the guest while the app is off screen.** A Compose composition survives backgrounding, so a plain `LaunchedEffect` loop keeps running in a pocket. The cockpit's git poll spawns a PRoot per pass, so it is scoped with `repeatOnLifecycle(RESUMED)` — against the 32-child phantom killer, and on a battery with no screen to show the answer on. `UsageTracker` stops for the same reason.
- **Restarting a dead server is bounded.** `RestartPolicy` allows five consecutive restarts with a growing backoff and then reports the failure instead of continuing. Without the bound a guest that could not come back — no code-server, a full disk, a `dpkg` stopped mid-unpack — was stopped and restarted every fifteen seconds for as long as the service lived, holding the wakelock, with the UI on "Reconnecting" and never saying what was wrong. Giving up does not stop the health polling, so a Repair that works is still noticed.
